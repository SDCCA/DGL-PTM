...
from dgl_ptm.network.local_attachment_tensor_Vedit import local_attachment_tensor 
...
def ptm_step(agent_graph, device, timestep, params):
    ...
    if params['step_type']=='custom':
        ...
        local_attachment_tensor(agent_graph, n_FoF_links = int(params['ratio']*agent_graph.number_of_nodes()), edge_prop = 'weight', p_attach=params['attachProb'][timestep])
        link_deletion(agent_graph, del_prob = params['del_prob'])
        global_attachment(agent_graph, ratio = params['ratio'])

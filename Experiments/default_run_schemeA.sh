...
log_file="default_schemeA_run_times.log"
> "$log_file" 
...
echo "Script: default_run_schemeA.sh"
...
for seed in "${seeds[@]}"
    do
        ...
        if [ "$counter" -ge "$restart" ] && [ "$counter" -le "$earlystop" ]; then
            ...
            variation="--seed $seed --steps 51 --agents 10000 --root_path output/SchemeA/default"
            python gpu_default_schemeA.py $variation 
            finish=$(date +%s)
            date=$(date)

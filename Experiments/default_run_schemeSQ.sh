...
log_file="default_schemeSQ_run_times.log"
> "$log_file" 
...
echo "Script: default_run_schemeSQ.sh"
...
for seed in "${seeds[@]}"
    do
        ...
        if [ "$counter" -ge "$restart" ] && [ "$counter" -le "$earlystop" ]; then
            ...
            variation="--seed $seed --steps 51 --agents 10000 --root_path output/SchemeSQ/default"
            python gpu_default_schemeSQ.py $variation 
            finish=$(date +%s)
            date=$(date)

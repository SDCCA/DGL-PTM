...
log_file="no_social_schemeB_run_times.log"
> "$log_file" 
...
echo "Script: no_social_run_schemeB.sh"
...
for seed in "${seeds[@]}"
    do
        ...
        if [ "$counter" -ge "$restart" ] && [ "$counter" -le "$earlystop" ]; then
            ...
            variation="--seed $seed --steps 51 --agents 10000 --root_path output/SchemeB/no_social"

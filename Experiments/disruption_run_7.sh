...
log_file="default_disrupt7_run_times.log"
> "$log_file" 
...
echo "Script: disruption_run_7.sh"
...
for seed in "${seeds[@]}"
    do
        ...
        if [ "$counter" -ge "$restart" ] && [ "$counter" -le "$earlystop" ]; then
            ...
            variation="--seed $seed --steps 51 --agents 10000 --root_path output/Disruption/disrupt_7 --shocks [0.7,5]"
